        ...
        <section class="flex flex-col my-2 items-center justify-center gap-y-6">
            ...
            <div class="w-full flex items-center justify-center">
                <div class="py-2 bg-green-700 text-white">
                    <form action="{{route('adminSeedDistribution.index')}}" method="GET" class="w-full">
                        @csrf
                    ...
                    @foreach ($farmers as $farmer)
                        <tr class="pt-10 odd:bg-slate-200 text-sm sm:text-base">
                            ...
                            <td class="flex items-center justify-center cursor-pointer">
                                <div onclick="showClaimSeedForm({{$farmer}})" class="{{$seasons != NULL && $seasons->Status === 'Active' && $farmer->RSBSA_No && !$farmer->is_claimed && $farmer->area->count() ? '' : 'hidden'}} bg-green-600 text-white font-bold text-base px-2 sm:px-3 py-1 rounded-sm" type="submit">
                                    Claim
                                </div>
                            </td>		
                        </tr>
...
<div id="claimSeedForm" class="hidden">
    <div class="h-screen w-screen bg-gray-500/50 fixed top-0 left-0 z-2 flex items-center justify-center">
        <form method="POST" id="claimSeedFormInputValue" action="{{route('adminSeedDistribution.claim')}}" class="p-3 w-full gap-2 text-gray-700 grid md:w-2/4 rounded shadow-md bg-white">
            @csrf
            @method('put')
