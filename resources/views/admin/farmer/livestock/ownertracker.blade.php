<x-app>
    <x-slot:title>
        Admin | Livestock Tracker Owner
    </x-slot:title>

    <x-admin.sidebar type="livestock trackers"/>
    ...
    <section class="w-full min-h-screen bg-green-700 p-5 overflow-y-auto">
        <section class="flex flex-col my-2 items-center justify-center gap-y-6">
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center gap-y-3 sm:gap-y-0 justify-center gap-x-3">
                {{-- <a class="py-2 px-6 bg-[#679f69] rounded flex items-center justify-between gap-x-4" href="">
                    <p class="text-white font-bold text-xs whitespace-nowrap">Change Livestock Owner</p>
                ...
                <div class="py-2 bg-green-700 text-white w-full">
                    <form action="{{route('adminLivestockOwnerTracker.index')}}" method="GET" class="w-full">
                        @csrf
...
<div id="showChangedForm" class="hidden">
    <div class="h-screen w-screen bg-gray-500/50 fixed top-0 left-0 z-2 flex items-center justify-center">
        <form method="POST" id="showChangedFormInputValue" action="{{route('adminLivestockOwnerTracker.changedOwner')}}" class="p-3 w-full gap-2 text-gray-700 grid md:w-2/4 rounded shadow-md bg-white">
            @csrf
            @method('put')
